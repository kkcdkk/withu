...
enum APIError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidResponse:
            return String(localized: "서버 응답 형식이 잘못됐어요.")
        case .server(let status, let detail):
            return String(localized: "서버 오류 \(status): \(detail)")
        case .decoding(let err):
            return String(localized: "디코딩 실패: \(err.localizedDescription)")
        case .transport(let err):
            return String(localized: "통신 실패: \(err.localizedDescription)")
        case .paymentRequired:
            return String(localized: "무료 횟수를 다 썼어요.")
        }
    }
}
...
extension Error {
    var koreanizedDescription: String {
        if let api = self as? APIError {
            ...
            case .paymentRequired:
                return String(localized: "무료 횟수를 다 썼어요. 충전하거나 구독해 주세요.")
            }
        }
        if let urlErr = self as? URLError {
            switch urlErr.code {
            case .notConnectedToInternet:
                return String(localized: "인터넷에 연결돼 있지 않아요. Wi-Fi 또는 셀룰러를 확인해 주세요.")
            case .timedOut:
                return String(localized: "응답이 너무 오래 걸려요. 잠시 후 다시 시도해 주세요.")
            case .cannotConnectToHost, .cannotFindHost:
                return String(localized: "서버에 연결할 수 없어요. 네트워크 또는 서버 상태를 확인해 주세요.")
            case .networkConnectionLost:
                return String(localized: "연결이 끊겼어요. 다시 시도해 주세요.")
            case .cancelled:
                return String(localized: "요청이 취소됐어요.")
            default:
                return String(localized: "네트워크 오류가 발생했어요. 다시 시도해 주세요.")
            }
        }
        // 기본 — 시스템 로컬라이즈된 메시지 (영문일 수 있음) 보다 깔끔한 한국어 폴백
        return String(localized: "오류가 발생했어요. 잠시 후 다시 시도해 주세요.")
    }
}
