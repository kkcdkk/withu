...
- 기능/수정 작업 브랜치: `codex/<짧은-작업명>` 형식을 기본으로 사용한다.
...
- GitHub issue는 코드, 문서, 테스트, 빌드 설정, repo 설정으로 고칠 수 있는
  작업만 만든다.
- Apple 시스템 제약, 로컬 Xcode 계정 로그인, 인증서 신뢰, 사용자 수동 QA 절차,
  App Store Connect 행정 작업처럼 저장소 변경으로 직접 고칠 수 없는 항목은 issue로
  남기지 않는다. 이런 항목은 최종 보고나 QA 메모에만 적는다.
- 작업 중 의미 있는 저장소/빌드/런타임 오류가 발견되고 같은 작업에서 해결하지
  못하더라도, 저장소 변경으로 고칠 수 있을 때만 GitHub issue를 만든다.
...
- 코드 변경 없이 재실행/환경 확인으로 해결된 경우에는 "코드 변경 없음"을 댓글에
  명확히 남긴 뒤 닫는다. 이후 같은 유형은 새 issue로 만들지 않는다.
